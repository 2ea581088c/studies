...
def generate_models(x, y, degs):
    ...
    """
    # TODO (y1[i] - y2[i])**2
    # for 1 deg, there are 2 coefficient; for 2 there are 3; for 3 there are 4 etc
    list = []
    for degree in degs:
        list.append(np.polyfit(x,y,degree))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        pylab.figure()
        pylab.plot(x,y,'b.', label = 'actual data')
        pylab.plot(x, np.polyval(model, x),'r-', label = 'model')
        
        if len(model) == 2:
            r2 = r_squared(y, np.polyval(model,x))
            
        pylab.title(str(len(model)) + 'degree model')
        pylab.xlabel('years')
        pylab.ylabel('temps')
            
        pylab.legend(title = str(r2), loc = 'best')
        pylab.show()
